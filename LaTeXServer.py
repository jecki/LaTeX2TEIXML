...
class LaTeXLanguageServerProtocol:
    ...
    def __init__(self):
        ...
        self.lsp_data = {
            ...
            'rootUri': '',
            'clientCapabilities': {},
            'serverInfo': { "name": "LaTeX-Server", "version": "0.1" },
            'serverCapabilities': {}
        }
        self.connection = None
        self.cpu_bound = LaTeXCPUBoundTasks(self.lsp_data)
        self.blocking = LaTeXBlockingTasks(self.lsp_data)
        self.lsp_table = gen_lsp_table(self, prefix='lsp_')
    ...
    def batch_job(self, argstr: str):
        from LaTeXParser import batch_process
        args = argstr.split(' ')
...
def run_server(host, port, log_path=None):
    """
    Starts a new LaTeXServer. If `port` is already occupied, different
    ports will be tried.
    """
    ...
    if grammar_src and not recompile_grammar(grammar_src, force=False,
        ...
        print('\nErrors while recompiling "%s":' % grammar_src +
              '\n--------------------------------------\n\n')
        with open('LaTeX_ebnf_ERRORS.txt', 'r', encoding='utf-8') as f:
            print(f.read())
        sys.exit(1)
    ...
    lsp_table = LaTeX_lsp.lsp_fulltable.copy()
    lsp_table.setdefault('default', LaTeX_lsp.simply_compile)
    LaTeX_server = Server(rpc_functions=lsp_table,
                        cpu_bound=LaTeX_lsp.cpu_bound.lsp_table.keys(),
                        blocking=LaTeX_lsp.blocking.lsp_table.keys(),
                        connection_callback=LaTeX_lsp.connect,
                        server_name="LaTeXServer",
                        strict_lsp=True)
    if log_path is not None:
        # echoing does not work with stream connections!
        LaTeX_server.echo_log = True if port >= 0 and host else False
        msg = LaTeX_server.start_logging(log_path.strip('" \''))
        if LaTeX_server.echo_log:  echo(msg)
    ...
    if port < 0 or not host:
        ...
        LaTeX_server.run_stream_server(reader, writer)
        return
...
def main():
    ...
    parser = ArgumentParser(description="Setup and Control of a Server for processing LaTeX-files.")
