...
"""LaTeXApp.py - a simple GUI for the compilation of LaTeX-files"""
...
class LaTeXApp(tkinter.Tk):
    def __init__(self):
        ...
        if 'html' in LaTeXParser.targets or 'HTML' in LaTeXParser.targets:
            self.target = 'html'
        elif len(LaTeXParser.targets) == 1:
            self.target = list(LaTeXParser.targets)[0]
        else:
            self.target = ''
        ...
        if not self.worker or self.on_cancel():
            ...
            if self.names:
                ...
                self.worker = threading.Thread(
                    target = LaTeXParser.batch_process,
...
if __name__ == '__main__':
    ...
    if not LaTeXParser.main(called_from_app=True):
        app = LaTeXApp()
        app.mainloop()
